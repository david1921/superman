...
class WcaxVariationTest < BrowserTest
  ...
  context "wcax with daily deal with variations on main deal page" do

    setup do
      @publishing_group = Factory(:publishing_group, :label => 'wcax')
      @publisher        = Factory(:publisher, :publishing_group => @publishing_group, :label => 'wcax-vermont', :enable_daily_deal_variations => true, :allow_voucher_shipping => true, :production_daily_deal_host => "localhost:#{@port}")
