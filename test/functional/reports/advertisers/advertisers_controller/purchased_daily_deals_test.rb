...
# hydra class Reports::Advertisers::AdvertisersController::PurchasedDailyDealsTest
      ...
      class PurchasedDailyDealsTest < ActionController::TestCase
        ...
        context "with off-platform purchase" do
          ...
          should "render xml and include the certificate" do
            get :purchased_daily_deals, :id => @advertiser.id, :dates_end => 1.day.from_now, :dates_begin => 1.day.ago, :format => 'xml'
            assert_response :ok
            assert_template 'purchased_daily_deals.xml'
        ...
        context "daily deal variations" do
          ...
          context "xml" do
            ...
            setup do
              get :purchased_daily_deals, :id => @advertiser.id, :dates_end => 1.day.from_now, :dates_begin => 1.year.ago, :format => "xml"
            end
            
            should "render appropriate data" do              
              @daily_deal.daily_deal_purchases.each do |ddp|
                ddp.daily_deal_certificates.each do |cert|                                    
                  assert_select "daily_deal_certificates daily_deal_certificate[daily_deal_certificate_id=#{cert.id}]" do
                    ...
                    assert_select "redeemed_at", :text => ""
                    ...
                    assert_select "value", :text => number_with_precision(ddp.value, :precision => 2)
                    assert_select "price", :text => number_with_precision(ddp.price, :precision => 2)
                    assert_select "purchased_price", :text => number_with_precision(cert.price, :precision => 2)
                  end
                end
              end
            end
